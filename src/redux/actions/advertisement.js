...
export const getAllAdvertisements = (token) => async dispatch => {
  ...
  try {
    const res = await axios.get(`${baseURL}/admin/get-all-advertisement`, config);
...
export const getSingleAdvertisement = (token, advertisementId) => async dispatch => {
    const config = {
      headers: {
        "Content-Type": "application/json",
        Authorization: token,
    ...
    try {
      const res = await axios.post(`${baseURL}/admin/get-single-advertisement`, body, config);
...
export const getFilteredAdvertisements = (input) => async dispatch => {
    ...
    try {
      const res = await axios.post(`${baseURL}/admin/get-all-advertisement`, body, config);
